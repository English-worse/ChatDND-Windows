...
项目缺少发布元数据、可重复的内存测量方式和最终 Windows 验收清单。

### 解决方案

为 App 项目添加产品版本元数据，新增内存测量脚本和本机验收清单，并完成 Release 测试、构建、发布、启动烟雾测试和 10 秒空闲内存测量。

### 修改明细

| 文件 | 改动点 | 改动类型 | 说明 |
|---|---|---|---|
| `src/ChatDND.App/ChatDND.App.csproj` | 发布元数据 | 配置变更 | 添加产品名、描述、公司和版本 0.1.0 |
| `tools/Measure-ChatDND.ps1` | 内存测量 | 新功能 | 采样工作集并输出平均、最大和最小值 |
| `docs/verification/windows-chat-dnd-acceptance.md` | 验收清单 | 新功能 | 记录自动检查和微信、QQ、钉钉等手工验收项 |

### 验证方法

运行 Release 测试、构建、发布，启动发布程序并用 `tools/Measure-ChatDND.ps1` 采样。

### 预期效果和潜在风险

已获得可回滚的发布包和约 49.4 MB 的 10 秒空闲工作集数据低于 80 MB 目标。长时间运行和多开聊天软件仍需按验收清单手工验证。
